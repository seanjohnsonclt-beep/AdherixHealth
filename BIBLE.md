...
### 2026-06-19 - Multi-modality expansion: medications, triggers, daily cadence

1. **`src/engine/medications.ts` rewritten** - expanded from 2 protocols to 20+ covering all 5 modalities (GLP-1, Pharmacotherapy, Metabolic, Bariatric, IBT). New `MedicationProtocol` type fields: `modality`, `category`, `isControlled`, `supplyDays`, `hasTitration`. `weekOffset` renamed to `daysOffset` everywhere. GLP-1 section: 11 drugs (semaglutide_wegovy, semaglutide_ozempic, semaglutide_oral, compounded_semaglutide, tirzepatide_zepbound, tirzepatide_mounjaro, compounded_tirzepatide, liraglutide_saxenda, liraglutide_victoza, dulaglutide, exenatide_er). Legacy `semaglutide` and `tirzepatide` keys preserved for backward compat with existing DB records. Pharmacotherapy: phentermine (CIV), topiramate, phentermine_topiramate/Qsymia (CIV), bupropion_naltrexone/Contrave, orlistat. Metabolic: metformin (90d supply), berberine. Bariatric: bariatric_supplements, bariatric_protein. IBT: ibt_no_medication placeholder. New helpers: `protocolsByModality()`, `controlledProtocols()`, `isRefillWindowOpen()`.
2. **`src/app/patients/new/EnrollForm.tsx` rewritten** - medication picker now filters by selected modality. IBT auto-assigns `ibt_no_medication` via hidden input (no picker shown). Controlled substance note in red for CIV drugs. Supply quantity field only shows when `protocol.supplyDays > 0 && protocol.supplyDays <= 30`. Starting dose only shows when `protocol.hasTitration`. Injection day field remains GLP-1 only.
3. **`src/engine/triggers.ts` critical SQL bug fixed** - missing comma between `phase_total_count` and `modality` columns was causing a SQL syntax error on every tick. The catch block fallback hardcoded `modality: 'glp1'` for every patient, meaning all bariatric, pharmacotherapy, metabolic, and IBT patients were silently running through the GLP-1 engine. Also fixed: added `p.injection_frequency` to SELECT, fixed `(p as any).modality` to `(patient as any).modality` (wrong variable name in loop context), updated Omit type, added `injection_frequency: null` to fallback patient mapping.
4. **`src/engine/conditions.ts` - daily confirmation cadence** - added `injection_frequency: string | null` to `PatientForEval` type. Gated `injection_confirmation_due` to weekly-only (returns false if `injection_frequency === 'daily'`). Added new `daily_med_confirmation_due` condition with tightening cadence: first 7 days - check every 2d; days 8-28 - every 3d; after day 28 - every 5d. Applies to liraglutide and Rybelsus patients.
5. **`src/lib/config.ts` - daily med templates and triggers** - added 4 templates (`trigger.daily_med_confirmation`, `trigger.daily_med_missed_followup`, `trigger.daily_med_escalation`, `trigger.rybelsus_routine_reminder`) and 3 triggers (`daily_med_confirmation`, `daily_med_noresponse`, `daily_med_escalation`). Templates landed in wrong array during initial write - fixed by removing from TRIGGERS and inserting before the `TRIGGERS` array.
6. **`src/engine/enroll.ts` minor fix** - type annotation changed from `weekOffset` to `daysOffset`; `medication ? 'weekly' : null` changed to `protocol?.frequency ?? null` so non-weekly drugs get the correct frequency at enrollment.
7. **`src/app/page.tsx` null byte fix** - file had binary null bytes causing TS2127 "Invalid character" errors. Fixed with Python `raw.replace(b'\x00', b'')`.
8. **`Adherix_SMS_Doctrine_Complete.docx` created** - complete SMS doctrine in the workspace folder. Covers all 5 modalities, all phases, all message templates, all triggers. 1069 paragraphs. Sections: How Engine Works, Keep (GLP-1), Bridge (Bariatric), Rx (Pharmacotherapy), Metabolic, IBT, Universal Logic, GLP-1 drug reference table.
9. **Last commit: `517f790 feat: daily GLP-1 confirmation cadence`** - all changes pushed to main.
...
2. **Section spacing** — Padding reduced across all three section namespaces: `.mkt-v2-section` `120px → 80px`, `.mkt-r-section` `96px → 72px`, `.mkt-section` `96px → 72px`. Hero padding cut from `152px/128px → 96px/80px`. Section-head bottom margin `72px → 48px`. Responsive breakpoints adjusted proportionally.
3. **Section contrast** — Alt-section backgrounds changed from `--mkt-paper-soft` (`#FAF6ED`) to `--mkt-sage-mist` (`#E0ECE9`) across all namespaces. Paper and Paper-soft are nearly identical warm beiges; sage-mist is visibly distinct while staying on-brand. Also added `border-bottom: 1px solid var(--mkt-line)` to all section rules as a hard separator.
4. **Pre-existing truncation fixed** — `globals.css` was truncated mid-line at the end of a `@media (max-width: 760px)` block (missing closing `}`). Fixed as part of this session.

### 2026-04-30 — Marketing site urgency pass + site ready for prospect traffic
1. **Primary CTA button contrast fixed** — `.mkt-btn--primary` darkened from `#3D7670` to `#1A3D38`. Added explicit `a.mkt-btn--primary` and `.mkt-page a.mkt-btn--primary` selectors with `!important` to override `.mkt-page a { color: inherit }` specificity issue that was making button text dark/invisible. Hover state: background drops away, text flips to `#1A3D38`, border appears — ghost reveal.
2. **Homepage hero rewritten consequence-first** — eyebrow, h1 ("Some of your patients are already drifting."), and subhead all reframed around cost-of-inaction rather than feature explanation.
3. **HomeConsequence panel added** — new dark `mkt-v2-section--ink` hard-stop between HomeProblem and HomePillars. `$252,000` at large Fraunces type, three brutal facts (35% / 72 hrs / 0 hours), forces cost-of-inaction before the solution is explained. New component `src/app/(marketing)/_components/sections/HomeConsequence.tsx`.
4. **Hero register differentiated across pages** — Homepage: consequence-first tension. Platform: operational ("Six phases. Four triggers. Every patient, every minute."). Drift Correction: response-in-motion ("By the time you notice, it's already corrected.").
5. **SiteFooter strengthened** — Drift Correction added to Product nav, contact email surfaced as a link, brand line sharpened. Footer was already present and complete; now matches the urgency register of the rest of the site.
6. **DC page redundant problem section removed** — "The signal most clinics miss" section (drift chart + 35% stat) dropped from `/drift-correction` — covered on homepage. `DriftChart` component also removed.
7. **Form routing confirmed** — "Book a demo" → `/pilot` form → `/api/pilot` → Resend → `seanjohnsonclt@gmail.com`. Verified working. Formspree (`NEXT_PUBLIC_FORMSPREE_CAPTURE_ID`) is a separate optional inline capture on the homepage CTA band — only renders if that env var is set in Vercel; currently not confirmed set.
8. **Site status** — considered ready for prospect traffic as of this session. Polish is sufficient; critical path (land → understand → book demo → email notification) is confirmed end-to-end.
